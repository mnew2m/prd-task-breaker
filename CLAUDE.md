...
- Jackson `write-dates-as-timestamps: false` → `LocalDateTime`을 ISO 문자열로 직렬화 (프론트엔드 `new Date()` 파싱 호환)
...
- 분석 히스토리: 앱 마운트 시 최근 목록 로딩, 접기/펼치기 패널로 최근 3개 표시, 클릭 시 과거 결과 로딩, 분석 완료 후 자동 갱신
...
## Out of Scope (MVP)
인증/인가, 파일 업로드, 결과 편집, 다국어, Rate limiting
