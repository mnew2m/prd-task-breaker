...
| 2026-03-17 | 타임스탬프 UTC 저장 + 프론트엔드 로컬 변환 (`LocalDateTime` → `Instant`, Flyway V3, api-spec `Z` 접미사) | — |
...
## 2026-03-15 - k6 CI 통합 및 모니터링 문서 강화

### 배경

CI/CD 파이프라인에 성능 테스트가 미통합 상태였고, `performance.yml`의 `--summary-export` 플래그 누락으로 아티팩트 업로드가 동작하지 않았다. 모니터링은 권장 수준 문서만 있고 구체적 설정 가이드가 없었으며, 프로덕션 P95 30초 달성 여부를 자동으로 검증하는 메커니즘이 부재했다.

### 수행 내용

**k6 CI 자동 통합 (`ci.yml` `performance` 잡)**
main push 시 자동 실행되는 `performance` 잡을 `ci.yml`에 추가했다. 백엔드 기동 → k6 실행 → threshold 실패 시 빌드 실패. k6의 기본 동작으로 `analysis_duration p(95) < 30000`, `error_rate == 0`, `http_req_failed == 0` threshold 중 하나라도 위반 시 exit code 1을 반환해 잡이 자동 실패한다. `--summary-export=k6-summary.json`으로 결과를 아티팩트로 보존.

**`performance.yml` 아티팩트 수정**
k6 실행 명령에 `--summary-export=k6-summary.json` 플래그를 추가해 결과 파일이 실제로 생성되도록 수정했다. 기존에는 플래그 없이 실행해 아티팩트 업로드가 항상 실패(`continue-on-error: true`로 무시)하고 있었다.

**모니터링 문서 강화 (`docs/deploy.md`)**
- UptimeRobot 무료 플랜 설정 가이드 (HTTP 모니터, 5분 간격, Slack 알림)
- Sentry 상세 설정: DSN 발급, 알림 규칙(첫 오류/에러 급증/P95 초과), 에러 필터링, Performance 대시보드 활용법
- Sentry Performance로 프로덕션 P95 자동 모니터링 커버 (`tracesSampleRate: 0.1` 이미 설정됨)
- 성능 목표 테이블에 검증 방법 컬럼 추가

**k6 설치 방식 수정**
초기 구현은 GPG 키서버를 통한 apt 설치 방식이었으나, CI 환경(`ubuntu-latest`)에서 `dirmngr` 미설치로 실패.
GitHub Releases API로 최신 버전 태그를 조회한 뒤 `k6-${VERSION}-linux-amd64.tar.gz` 바이너리를 직접 다운로드하는 방식으로 변경했다.
