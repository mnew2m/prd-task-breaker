...
- 히스토리 테스트: 분석 완료 후 결과 화면(`hasResult=true`)에서는 `AnalysisHistory`가 렌더링되지 않음. `+ 새 분석` 클릭으로 입력 화면 복귀 후 히스토리 패널 확인하도록 수정
- 피드백 테스트: `getByRole('button', { name: '👍' })`가 `aria-label="유용함"`과 매칭 안 됨. Playwright는 accessible name으로 `aria-label`을 우선 사용하므로 `{ name: '유용함' }`으로 변경
