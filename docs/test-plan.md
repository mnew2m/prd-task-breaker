...
> **한계**: CI `performance` 잡은 **dev 프로필(MockAiClient)** 기반으로 실행된다. MockAiClient는 고정 JSON을 즉시 반환하므로 P95가 수 ms 수준 — k6 threshold(`p(95)<30000`)는 구조적으로 실패하지 않는다. 따라서 이 잡은 **회귀 방지(k6 스크립트 문법·HTTP 연결 이상 감지)** 용도이며, 실제 Claude API 응답 시간 검증은 아니다.
>
> 실제 프로덕션 P95 검증은 `performance.yml` 수동 트리거(프로덕션 URL 지정) 또는 Sentry Performance 알림(`Transaction duration p95 > 30s`)으로 수행한다.
...
- `performance`: main push 시 실행 — 실제 백엔드 기동 후 k6 부하 테스트 (P95 < 30s threshold 실패 시 빌드 실패); `k6-results` 아티팩트 저장. **단, dev 프로필(MockAiClient) 기반으로 실제 AI 응답 시간 미측정**
