...
| 백엔드 의존성 취약점 (CVE) | OWASP Dependency Check (`dependencyCheckAnalyze`) | CI 매 실행 (Critical CVSS≥9 빌드 실패) |
...
- `backend-security`: OWASP Dependency Check (Critical CVSS≥9 시 빌드 실패, NVD 캐시 적용)
