...
| 백엔드 의존성 취약점 (CVE) | OWASP Dependency Check (`dependencyCheckAnalyze`) | 주 1회 자동 (Critical CVSS≥9 빌드 실패) |
...
- ~~`backend-security`~~: ci.yml에서 제거됨 → `security.yml`로 분리 (매주 월요일 09:00 KST, 수동 실행 가능)
