...
| 항목 | 목표값 | 검증 방법 |
|------|--------|-----------|
| AI 분석 응답 P95 | 30초 이내 | `performance.yml` 수동 트리거(프로덕션 URL) + Sentry P95 알림 |
...
> **한계**: CI 잡은 **dev 프로필(MockAiClient)** 기반이므로 실제 Claude API 응답 시간을 측정하지 않는다. k6 threshold는 구조적으로 실패하지 않으며, 이 잡의 실질 목적은 회귀 방지(k6 스크립트 이상, HTTP 연결 오류 감지)다.

### 프로덕션 실제 성능 검증

두 가지 방법으로 실제 프로덕션 P95를 검증한다:

1. **`performance.yml` 수동 트리거**: GitHub Actions → Performance Test → Run workflow → `base_url`에 Railway URL 입력
2. **Sentry Performance 알림**: DSN 설정 완료 시 "Transaction duration p95 > 30s" 알림 규칙이 자동 감지 (설정 방법: [Sentry 상세 설정 가이드](#sentry-상세-설정-가이드) 참조)
